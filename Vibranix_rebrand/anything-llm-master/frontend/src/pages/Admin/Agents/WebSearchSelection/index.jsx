...
// prefer runtime logo from LogoContext for placeholder entries
...
const SEARCH_PROVIDERS = [
  {
    ...
    value: "none",
    // leave null so we can fall back to the instance logo at render-time
    logo: null,
...
export default function AgentWebSearchSelection({
  ...
  const { logo: instanceLogo } = useLogo();
  ...
  useEffect(() => {
    ...
    ).map((p) => ({ ...p, logo: p.logo || instanceLogo }));
    setFilteredResults(filtered);
  }, [searchQuery, selectedProvider, instanceLogo]);
  ...
  const displayLogo = (selectedSearchProviderObject && (selectedSearchProviderObject.logo || instanceLogo)) || instanceLogo;
  return (
    <div className="p-2">
            ...
            {searchMenuOpen ? (
              ...
              >
                <div className="flex gap-x-4 items-center">
                  <img
                    src={displayLogo}
                    alt={`${selectedSearchProviderObject?.name ?? "provider"} logo`}
